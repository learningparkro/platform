...
def main():
    ...
    for row in reader:
        ...
        email = row[3]
        try:
            ret = wouso.utils.user_util.update_user_by_email(email, username=username)
        except:
            print "Failed changing username for email %s." %(email)
            continue
        if ret:
            print "Successfully changed username for email %s." %(email)
        else:
            print "Failed changing username for email %s." %(email)
